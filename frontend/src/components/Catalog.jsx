<!doctype html>
<html lang="ru">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="/static/styles.css" />
    <title>Интернет магазин - Незабудка</title>
</head>
<body>
    <header>
        <h1>Незабудка</h1>
        <nav>
            <ul>
                <li><a href="/">Главная</a></li>
                <li><a href="cart">Корзина</a></li>
                <li><a href="profile">Профиль</a></li>
            </ul>
        </nav>
    </header>

    <main>
        <section class="search-section">
            <form method="get" action="/">
                <div>
                    <label for="search-query">Поиск:</label>
                    <input
                        type="search"
                        id="search-query"
                        name="q"
                        placeholder="Например, DDR4..."
                        value="{{ q }}"
                    />
                </div>
                <div>
                    <label for="category">Категория:</label>
                    <select id="category" name="category">
                        <option value="all" {% if category_selected == 'all' %}selected{% endif %}>
                            Все категории
                        </option>
                        {% for category in categories %}
                        <option value="{{ category.id }}"
                                {% if category_selected == category.id|string %}selected{% endif %}>
                            {{ category.name }}
                        </option>
                        {% endfor %}
                    </select>
                </div>
                <button type="submit">Найти</button>
            </form>
        </section>

        <section class="products">
            <h2>Найденные товары</h2>
            <div class="products-container">
                {% for product in products %}
                <article class="product-card">
                    <img src="{{ product.image_url or 'https://via.placeholder.com/320x250?text=Нет+фото' }}"
                         alt="{{ product.name }}">
                    <h3>{{ product.name }}</h3>
                    <div class="price-form-container">
                        <div class="price">{{ product.price | currency }} ₽</div>
                        <form class="order-form" method="post" action="/order">
                            <input type="hidden" name="product_id" value="{{ product.id }}">
                            <button type="submit" class="order-btn">+</button>
                        </form>
                    </div>
                    <div class="additional-information">
                        <p>{{ product.description or '' }}</p>
                        {% if product.characteristics %}
                        <h4>Характеристики:</h4>
                        <table class="specifications">
                            <tbody>
                                {% for key, value in product.characteristics.items() %}
                                <tr>
                                    <td>{{ key }}:</td>
                                    <td>{{ value }}</td>
                                </tr>
                                {% endfor %}
                            </tbody>
                        </table>
                        {% endif %}
                    </div>
                </article>
                {% else %}
                <p>Товары не найдены.</p>
                {% endfor %}
            </div>
        </section>
    </main>

    <footer>
        <p>&copy; 2026 ННГУ им Лобачевского. Все права защищены.</p>
    </footer>
</body>
</html>